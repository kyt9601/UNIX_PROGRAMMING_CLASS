...
int f_rmall_file(const char *name, const struct stat *status, int type)
{
        if (type == FTW_F)
		remove(name);
...
int f_rmall_dir(const char *name, const struct stat *status, int type)
{
        if ((type == FTW_D || type == FTW_DNR) && name[0] != '.')
                remove(name);
...
void rmall_m(char **s)
{
	ftw(s[1], f_rmall_file, 1);
	ftw(s[1], f_rmall_dir, 1);
	remove(s[1]);
}
